...
SparkleFormation.dynamic(:load_balancer) do |_name, _config={}| 
	...
	parameters do 
		...
		set!("#{lb_name}_protocol".to_sym) do 
			type 'String'
			description 'Protocol for the listener of the Load Balancer resource'
			default _config[:protocol] || 'tcp'
		end 

		set!("#{lb_name}_instance_port".to_sym) do 
			type 'String' 
			description 'Instance Port for the listener of the Load Balancer resource'
			default _config[:instance_port] || 80
		end 

		set!("#{lb_name}_instance_protocol".to_sym) do 
			type 'String'
			description 'Instance Protocol for the listener of the Load Balancer resource'
			default _config[:instance_protocol] || 'tcp'
		end 

		set!("#{lb_name}_target".to_sym) do 
			type 'String'
			description 'HealthCheck Target for the Load Balancer resource'
		...
		set!("#{lb_name}_unhealthy_threshold".to_sym) do 
			type 'Number'
			description 'HealthCheck UnhealthyThreshold for the Load resource'
			default _config[:healthy_threshold] || '2'
		end 

		set!("#{lb_name}_interval".to_sym) do 
			type 'Number'
			description 'HealthCheck Interval for the Load Balancer resource'
			default _config[:interval] || '10'
		end 

		set!("#{lb_name}_timeout".to_sym) do 
			type 'Number'
			description 'HealthCheck Timeout for the Load Balancer resource'
	...
	outputs do 
		set!("#{_name}_autoscaling_group_load_balancer_resource_name".to_sym) do 
			description 'Name of the Load Balancer resource'
			value ref!(lb_name.to_sym)
		end 

		port do 
			description 'Listening port for the Load Balancer resource'
